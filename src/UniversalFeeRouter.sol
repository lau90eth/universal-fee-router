...
contract UniversalFeeRouter {
    ...
    function getClaimable(address recipient, address token)
    ...
    function _distribute(address token, uint256 amount) private {
        ...
        for (uint256 i; i < len; ++i) {
            uint256 share = (i == len - 1)
                ? remaining
                : (amount * _splits[i].bps) / BPS_DENOMINATOR;
     ...
     */
    function _tryPush(address token, address recipient, uint256 amount)
        private returns (bool)
    {
        if (token == ETH_ADDRESS) {
            ...
            (bool ok, bytes memory data) = token.call{gas: ERC20_PUSH_GAS}(
